...
I will use sparse one-hot vectors
...
def to_one_hot_vec(word, corpus_with_idx):
    indices = torch.tensor([[0], [corpus_with_idx[word]]], dtype=torch.long)
    vals = [1.0]
...
class ContextTargetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        context_vec_list = []
        for i in range(2*N):
            context_word_vec = to_one_hot_vec(context[i], corpus_with_idx)
            context_vec_list.append(context_word_vec)
        
        # target_vec = to_one_hot_vec(target, corpus_with_idx)
        target_idx = torch.tensor([corpus_with_idx[target]])
...
def training_loop(model, optimizer, loss_fn, train_loader, val_loader,
    ...
    for epoch in range(n_epoch):
        ...
        #     context_vec_list, target_idx = train_dataset[i]
...
correct = 0
total = len(target_list_train)
for i in range(total):
    context_vec_list, target_idx = train_dataset[i]
    log_probs = model1(context_vec_list)
    _, max_prob_idx = log_probs.max(dim=1)
    
    if target_idx == max_prob_idx:
        correct += 1
        

print(f"Accuracy (training data): {correct/total}")