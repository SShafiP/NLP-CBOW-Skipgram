...
in_train = target_list_train
in_val = target_list_val

out_train = context_list_train
out_val = context_list_val
...
def training_loop(model, optimizer, loss_fn, in_train, out_train, in_val,
                  out_val, n_epoch):
    out_idx_train = torch.zeros([1, 2*N], dtype=torch.long)
    out_idx_val = torch.zeros([1, 2*N], dtype=torch.long)
    for epoch in range(n_epoch):
        train_loss_epoch = 0
        for i in range(len(in_train)):
            in_train_vec = to_one_hot_vec(in_train[i], corpus_with_idx)
            out_train_vec_p = model(in_train_vec)
            
            for j in range(2*N):
                out_idx_train[:, j] = corpus_with_idx[out_train[i][j]]
            
            train_loss_sample = 0
            for j in range(2*N):
                # print(out_idx_train)
                # print(out_idx_train[:, j])
                # print(out_train_vec_p[:, (j*V):((j + 1)*V)].shape)
        ...
        with torch.no_grad():
            for i in range(len(in_val)):
                in_val_vec = to_one_hot_vec(in_val[i], corpus_with_idx)
                out_val_vec_p = model(in_val_vec)
            
                for j in range(2*N):
                    out_idx_val[:, j] = corpus_with_idx[out_val[i][j]]
