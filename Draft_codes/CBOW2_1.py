...
Data: A paragraph from "Conquering the Physics GRE"
Training input is a 2*N-of-2*N*V encoded vector (fed to the neural network as
an N_sample-by-2*N*V tensor)
...
def training_loop(model, optimizer, loss_fn, in_train, out_train, in_val,
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len(in_train)):
            in_train_vec = context_to_vec(in_train[i], corpus_with_idx)
            out_train_vec_p = model(in_train_vec)
            out_idx_train = torch.tensor([corpus_with_idx[out_train[i]]],
                                         dtype=torch.long)
        ...
        with torch.no_grad():
            for i in range(len(in_val)):
                in_val_vec = context_to_vec(in_val[i], corpus_with_idx)
                out_val_vec_p = model(in_val_vec)
                out_idx_val = torch.tensor([corpus_with_idx[out_val[i]]],
                                           dtype=torch.long)
